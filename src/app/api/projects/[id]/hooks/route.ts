...
export async function GET(_req: Request, { params }: Params) {
  ...
  return json({ hooks: listDeployHooks(project.id) });
}
...
export async function POST(req: Request, { params }: Params) {
  ...
  const hook = createDeployHook(project.id, name, branch);
  recordActivity(project.id, user.name, "hook.created", `Deploy hook “${name}” created for ${hook.branch}`);
